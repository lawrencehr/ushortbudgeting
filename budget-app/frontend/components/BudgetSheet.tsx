...
export default function BudgetSheet({ categoryId }: BudgetSheetProps) {
    const [categories, setCategories] = useState<BudgetCategory[]>([]);
    const [loading, setLoading] = useState(true);
    const [activeTab, setActiveTab] = useState<"ATL" | "BTL">("ATL");
    ...
    const handleAdd = async (item: CatalogItem) => {
        try {
            ...
            await addBudgetLineItem(targetGroupingId, item.description, item.default_rate, item.is_labor);
            loadBudget();
        } catch (err) {
            console.error(err);
        ...
        <div className="space-y-4 text-xs">
            ...
            <div className="space-y-8">
                ...
                {filteredCategories.map((cat, _) => (
                    <div key={cat.id} className="bg-white shadow-lg rounded-xl overflow-hidden border border-slate-200">
                        <div className="bg-slate-800 text-white px-4 py-2.5 flex justify-between items-center text-sm font-bold tracking-tight">
                            <span>{cat.code} - {cat.name}</span>
                            <span className="bg-slate-700 px-3 py-1 rounded text-emerald-400 font-mono">Total: ${cat.total.toLocaleString()}</span>
                        </div>
